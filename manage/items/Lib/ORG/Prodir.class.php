...
class Prodir{
	...
	function option($groupid,$cid,$pid,$guid='0'){
		$list=M("prodir")->where("pid='".$cid."' and groupid='".$groupid."'")->order("sort,time desc")->select();
		if($list){
			foreach($list as $value){
				...
				$z_value=M("prodir")->where("pid='".$value['guid']."' and groupid='".$groupid."'")->select();
				if($z_value){
					$this->listfj.="┇";
	...
	function Title($pid){
		$find=M("prodir")->where("guid='".$pid."'")->find();
		if(!$find){
			return '';
		}
		if($find['pid']!='0'){
		...
		$this->Titlec.= '<a href="__APP__/Prodir/index/pid/'.$find['guid'].'" class="current">'.$find['title'].'</a>';
		return $this->Titlec;
	}
