...
neverModules.modules.slider.prototype = {
	_defaultInitializer:function () {
		...
		this._wrapper = null;
		console.log(this.parent,this.targetId);
	...
	_moveTo:function (evt) {
		with (this) {
			...
			console.log(evt,this.parent.event);
	...
	_initMoveSlider:function (evt) {
		...
		with (this) {
			...
			fireStart();
			this.parent.document.onmousemove = function (evt) {
				_self._changeHandle(evt);
			}
			this.parent.document.onmouseup = function (evt) {
	...
	_changeHandle:function (evt) {
		with (this) {
			...
			/*if(x)*/
			{
				...
				}//添加改变的值到输入框,这里加setTimeout解决消息堵塞问题 add by tangzz
				fireChange();
			}
			/*else
			 {
